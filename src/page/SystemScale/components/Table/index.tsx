...
const treeToListDepth = (
  ...
  tree.forEach((item, index) => {
    ...
    if (children.length) {
      ...
      const md5 = CryptoJS.MD5(JSON.stringify({ ...append, index })).toString();
      append[`md5_${depth}`] = md5;
      result = result.concat(treeToListDepth(children, append, 1 + depth));
    } else {
...
export const TableWrap = () => {
  const columns: TableColumnsType<Partial<DataType>> = [
    {
      ...
      onCell(record: Partial<DataType>, index) {
        const { md5_0 } = record;
        if (
          (list.findIndex((item) => item.md5_0 === md5_0) === index) ===
          false
        ) {
          return { rowSpan: 0 };
        }
    ...
    },
    {
      ...
      onCell(record: Partial<DataType>, index) {
        const { md5_1 } = record;
        if (
          (list.findIndex((item) => item.md5_1 === md5_1) === index) ===
          false
        ) {
          return { rowSpan: 0 };
        }
  ...
  return (
    <Table columns={columns} bordered dataSource={list} pagination={false} />
  );
};
