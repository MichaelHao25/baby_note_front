...
const EvalModal = memo<Props>(({ onRef, callBack }) => {
  ...
  const showDrawer = (info: TeacherItem) => {
    ...
    // fetchEvalList();
    refetchInfo(info?.id);
    refetchAuditDetail(info?.id);
    // refetchEvaluateInfo(info?.criterionId);
    setCurType(info);

    setEvalType(`${info?.parentName} / ${info?.scores?.[0]?.name}`);
    setTitle(info?.scores?.[0]?.name);
    setOpen(true);
  };
        ...
        <div className="flex justify-end">
          <div>
            <Button
              ...
              type="primary"
              onClick={() => {
                callBack?.('audit', { id: curType.id, type: 1, score });
              }}
            >
              通过
            </Button>
            <Button
              className="ml-[15px]"
              onClick={() => {
                callBack?.('audit', { id: curType.id, type: 0, score });
              }}
            >
              不通过
            </Button>
